...
function isPalindrome(str) {
  const string = str.replace(/[^a-z0-9]/gi, '').toLowerCase();
...
function encodeToRot13(str) {
  return str
    ...
    .map((elem) => {
      if (elem >= 'A' && elem <= 'Z') {
        return String.fromCharCode(((elem.charCodeAt(0) - 65 + 13) % 26) + 65);
      }
      if (elem >= 'a' && elem <= 'z') {
        return String.fromCharCode(((elem.charCodeAt(0) - 97 + 13) % 26) + 97);
      }
      return elem;
    })
    .join('');
...
function getCardId(value) {
  ...
  const cardID = arrCard.indexOf(value);
